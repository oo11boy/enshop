    ...
    <div className="showcart">
      <div className="completecart">
        ...
        {itemsToRender.map((item) => (
          <div className="cartbody" key={item.id}>
            ...
            <div className="informcart">
              <Link to={"product/" + item.id} className="text-primary text-decoration-none">
                {item.name}
              </Link>
        ...
        <div className="undercart">
          ...
          <h3 className="viewpaycart text-white">جمع کل: {cartinfo.totalprice().toLocaleString()} تومان</h3>
        </div>
      </div>
    </div>
  );
